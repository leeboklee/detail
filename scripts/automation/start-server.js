...
async function startServer(isCI = false) {
  ...
  const serverProcess = spawn('npx', ['next', 'dev', '-p', '3900'], {
    stdio: 'inherit',
    shell: true,
    cwd: path.resolve(__dirname, '..')
  });
  ...
  const checkServerReady = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        log.info('\ud83c\udf10 http://localhost:3900');
        
        // CI 모드가 아닐 때만 브라우저 자동 열기 (비활성화)
        if (!isCI) {
          log.info('브라우저 자동 열기 비활성화됨. 수동으로 http://localhost:3900 접속하세요.');
        }
      } else {
        // 서버는 켜졌지만 아직 준비 안됨
...
async function main() {
  ...
  try {
    ...
    // 3. 포트 확인
    const portAvailable = await checkPort(3900);
    if (!portAvailable) {
      log.error('\ud3ec\ud2b8 3900\uc744 \uc0ac\uc6a9\ud560 \uc218 \uc5c6\uc2b5\ub2c8\ub2e4.');
      process.exit(1);
    }
