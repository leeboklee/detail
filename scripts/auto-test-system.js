...
async function testPreviewFunctionality() {
  ...
  const checks = [
    { name: 'div 기반 렌더링', pattern: /div.*ref.*previewRef/, message: 'div 기반 미리보기 구현됨' },
    ...
    { name: '실시간 업데이트', pattern: /useEffect.*previewContent/, message: '실시간 업데이트 구현됨' }
  ];
...
async function testErrorHandling() {
  ...
  const checks = [
    ...
    { name: '네트워크 오류 감지', pattern: /fetch.*response\.ok/, message: '네트워크 오류 감지됨' },
